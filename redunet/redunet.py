...
class ReduNet(nn.Sequential):
    ...
    def batch_forward(self, inputs, batch_size=1000, loss=False, cuda=True, device=None):
        ...
        for i in range(0, inputs.shape[0], batch_size):
            print('batch:', i, end='\n')
            batch_inputs = inputs[i:i+batch_size] #torch.Size([25, 1, 28, 28]) 变成torch.Size([10, 1, 28, 28])
            print('inputs', inputs.size(), 'batch_inputs', batch_inputs.size())
            if device is not None:
                batch_inputs = batch_inputs.to(device)
            elif cuda:
                batch_inputs = batch_inputs.cuda()
            batch_outputs = self.forward(batch_inputs, loss=loss) #torch.Size([10, 35, 28, 28])
            print('batch_outputs',batch_outputs.size())
            outputs.append(batch_outputs.cpu()) #从[]变成四维列表，
            print('cat',torch.cat(outputs).size())
        return torch.cat(outputs)  #[20, 35, 28, 28]
